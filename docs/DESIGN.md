...
The checkpoint control lists the three newest persisted Agent runs after reload. Agent prompts receive bounded, chronological context from the active conversation, alongside bounded workspace instructions and attachments, so prior project decisions remain available without flooding the model window.
...
The context menu can attach language-service definition locations, references, and hover type information alongside files, symbols, and diagnostics. The composer shows a compact active-file chip and accepts pasted or dropped image files as bounded attachments. Image payloads are sent as typed vision parts only to models that advertise image input; OpenAI-compatible providers receive image URLs and Ollama receives its `images` array. If no configured vision-capable route exists, the run fails with an explicit setup instruction instead of dropping the image. The current OpenCode ACP harness remains text-first and does not silently accept pasted binaries.
