...
- Empty provider completions now fall back as invalid routes. Empty Agent runs automatically start one fresh session against the next Auto Stack route and offer a compact next-route retry if the outcome remains empty; Agent runs cancel after a configurable inactivity window (default 120 seconds) rather than waiting indefinitely.
- A dedicated Nexus Agent view reports profile, workspace trust, route stack size, MCP server readiness, and persisted checkpoint availability. Pasted/dragged images become removable attachment chips and are dispatched only to models that advertise image input; text-only routes fail closed with setup guidance.
